...
contract("Token", function(accounts) {
  ...
  it("Owner Mining tokens. Total supply cap.", async function() {
    await tokenInstance.mint(user_1.address, initialBalances[0], {
      from: owner
    });
    assert.equal(
    ...
    await tokenInstance.mint(user_1.address, initialBalances[1], {
      from: owner
    });
    assert.equal(
      (await tokenInstance.balanceOf(user_1.address)).toNumber(),
      (user_1.balance += initialBalances[1]),
      "Owner is second mining user_1 1000"
    );

    await tokenInstance.mint(user_2.address, initialBalances[2], {
      from: owner
    });
    assert.equal(
      (await tokenInstance.balanceOf(user_2.address)).toNumber(),
      (user_2.balance += initialBalances[2]),
      "Owner is second mining user_2 1000"
    );

    await tokenInstance.mint(user_3.address, initialBalances[3], {
      from: owner
    });

    assert.equal(
      (await tokenInstance.totalSupply.call()).toNumber(),
      initialBalances.reduce(function(a, b) {
        return a + b;
      }),
      "Total supply"
    );
  ...
  it("Swindler mining tokens", async function() {
    try {
      await tokenInstance.mint(user_2.address, initialBalances[0], {
        from: swindler
      });
    } catch (e) {
  ...
  it("Initial Pausable tokens and transfer token", async function() {
    ...
    try {
      await tokenInstance.transfer(user_3.address, initialBalances[3], {
        from: user_1.address
      });
    } catch (e) {
      assert.ok(true);
  ...
  it("Unpaused tokens transfer token", async function() {
    ...
    assert.equal(
      (await tokenInstance.balanceOf(user_3.address)).toNumber(),
      user_3.balance,
      "Transfer to user 3"
    );
    assert.equal(
  ...
  it("Investors counter", async function() {
    ...
    await tokenInstance.mint(user_3.address, initialBalances[2], {
      from: owner
    });
