...
async function actionManagerQuery(
  actionName,
  data,
  funcName = "execute",
  address = "",
  actionManagerMethod = "execute"
) {
  ...
  await ActionManagerInstance[actionManagerMethod](
    web3.utils.toHex(actionName),
    _data
  );
}
...
contract("NousCore", async function(accounts) {
  ...
  it("Add Action templates. Action Create New Fund", async () => {
    ...
    for (let item in templatesList) {
      assert.equal(
        templatesList[item].address,
        await instanceList["TemplatesDb"].template(item, 0),
        "Instance list not equal templates Db"
      );
    }
    ...
    console.log(
      "STEP 1 TPLSnapshotDb",
      await actionManagerQuery(
        "TPLSnapshotDb",
        _data,
        "create",
        undefined,
    ...
    console.log(
      "STEP 2 ActionCreateCompOEFund2",
    ...
    console.log(
      "STEP 3 ActionCreateCompOEFund3 ",
    ...
    data = [accounts[1]];
    console.log(
      "STEP 4 ActionCreateActionsOEFund1",
      await actionManagerQuery("ActionCreateActionsOEFund1", data)
    );

    //STEP 5 create Fund
    data = [accounts[1]];
    console.log(
      "STEP 4 ActionCreateActionsOEFund2",
      await actionManagerQuery("ActionCreateActionsOEFund2", data)
    );

    // get all tpls
    tpls = await instanceList["TemplatesDb"].getTplContracts(
      accounts[1],
      web3.utils.toHex("contracts")
    );
    ...
    let acts = await instanceList["TemplatesDb"].getTplContracts(
      accounts[1],
      web3.utils.toHex("actions")
    );
    //console.log("acts", acts);

    let fundAddr = await instanceList["ProjectDb"].getOwnerFundLast(
      accounts[0]
    );
    ...
    console.log(
      "ActionAddActions",
      (await projectManager.execute("ActionAddActions", bytes)).tx
    );
  });
});
